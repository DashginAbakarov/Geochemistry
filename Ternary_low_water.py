...
df_dropped = df.drop(['Cl %', 'SO4 %', 'HCO3 %', '(K+Na) %', 'Mg %', 'Ca %',
       'Total (g/l)', 'total anionsl (g/Mol)', 'total  kations (g/Mol)','Cl .1', 'SO4 [-2].1', 'HCO3 .1', '(K+Na) .1', 'Mg [+2] .1',
       'Ca [+2] .1'], axis=1)
...
df_dropped.columns = ['N','Name', 'Anions (g/l)', 'Total Ions', 'Kations (g/l)', 'Anion (mol/l)',
       'Kation (mol/l)', 'Cl ', 'SO4', 'HCO3 ', '(K+Na) ', 'Mg ',
       'Ca']
...
df_nor_anion = df_dropped.iloc[:, 7:10] = df_dropped.iloc[:, 7:10].div(df_dropped.iloc[:, 7:10].sum(axis=1), axis=0)*100
df_nor_cation = df_dropped.iloc[:, 10:13] = df_dropped.iloc[:, 10:13].div(df_dropped.iloc[:, 10:13].sum(axis=1), axis=0)*100
print(df_nor_anion)
print(df_nor_cation)
...
# plt.savefig(r'C:\Users\dabakarov\Downloads\lakes\dashgin_thesis_phd\My_thesis\Pyhton\Ternary Low water.jpeg', format='jpeg', dpi=300)
